...
describe('repRange utilities', () => {
  describe('formatRepRange', () => {
    it('should format range "8-12 reps"', () => {
      expect(formatRepRange(8, 12)).toBe('8-12 reps');
    });

    it('should format equal min/max as single value', () => {
      expect(formatRepRange(5, 5)).toBe('5 reps');
    });
    ...
    it('should handle null min, use max', () => {
      expect(formatRepRange(null, 10)).toBe('10 reps');
    });

    it('should handle null max, use min', () => {
      expect(formatRepRange(8, null)).toBe('8 reps');
    });

    it('should handle undefined min, use max', () => {
      expect(formatRepRange(undefined, 10)).toBe('10 reps');
    });

    it('should handle undefined max, use min', () => {
      expect(formatRepRange(8, undefined)).toBe('8 reps');
    });

    it('should handle null/undefined mix', () => {
      expect(formatRepRange(null, undefined)).toBe('');
      expect(formatRepRange(undefined, null)).toBe('');
    });

    it('should format reverse range (unlikely but handled)', () => {
      expect(formatRepRange(12, 8)).toBe('12-8 reps');
    });

    it('should format single digit ranges', () => {
      expect(formatRepRange(1, 3)).toBe('1-3 reps');
    });

    it('should format large ranges', () => {
      expect(formatRepRange(10, 50)).toBe('10-50 reps');
    });

    it('should be consistent', () => {
      const result1 = formatRepRange(8, 12);
      const result2 = formatRepRange(8, 12);
